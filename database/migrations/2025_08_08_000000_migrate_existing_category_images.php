...
return new class extends Migration
{
    ...
    private function generateNormalizedNames(string $categoryName): array
    {
        ...
        $name2 = trim($name2, '_');
        if (! empty($name2) && ! in_array($name2, $normalizedNames)) {
            $normalizedNames[] = $name2;
        }

        // Estrategia 3: solo espacios a guiones
        $name3 = strtolower(str_replace(' ', '-', $categoryName));
        if (! empty($name3) && ! in_array($name3, $normalizedNames)) {
            $normalizedNames[] = $name3;
        }

        // Estrategia 4: solo espacios a guiones bajos
        $name4 = strtolower(str_replace(' ', '_', $categoryName));
        if (! empty($name4) && ! in_array($name4, $normalizedNames)) {
            $normalizedNames[] = $name4;
        }

        // Estrategia 5: solo caracteres alfanuméricos
        $name5 = strtolower(preg_replace('/[^a-z0-9]/i', '', $categoryName));
        if (! empty($name5) && ! in_array($name5, $normalizedNames)) {
            $normalizedNames[] = $name5;
        }

        // Estrategia 6: tal como está, solo minúsculas
        $name6 = strtolower($categoryName);
        if (! empty($name6) && ! in_array($name6, $normalizedNames)) {
            $normalizedNames[] = $name6;
        }
        ...
        $name7 = trim($name7, '-');
        if (! empty($name7) && ! in_array($name7, $normalizedNames)) {
            $normalizedNames[] = $name7;
        }

        // Estrategia 8: tal como está original
        if (! empty($categoryName) && ! in_array($categoryName, $normalizedNames)) {
            $normalizedNames[] = $categoryName;
        }
