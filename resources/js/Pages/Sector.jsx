...
export default function Sector({ productos, tagParent, tags, marcas, allTagParents }) {
    ...
    useEffect(() => {
        ...
        const sectorId = urlParts[urlParts.length - 1];
        console.log('Sector seleccionado ID:', sectorId);
        console.log('Objeto completo del sector:', tagParent);
    }, []);

    useEffect(() => {
        if (tagParent) {
            console.log('TagParent prop recibida:', tagParent);
        }
    }, [tagParent])
    ...
    const [isTagDropdownOpen, setIsTagDropdownOpen] = useState(true);
    const [selectedBrand, setSelectedBrand] = useState(null);
    const [selectedTag, setSelectedTag] = useState(null);
    const [filteredProducts, setFilteredProducts] = useState(productos);

    // Efecto para filtrar productos por marca y tag
    useEffect(() => {
        ...
        if (selectedTag && selectedTag !== 'all') {
            filtered = filtered.filter(product => 
                product.tags && product.tags.some(tag => 
                    String(tag.id_tag) === String(selectedTag)
                )
            );
        }
        
        setFilteredProducts(filtered);
    }, [selectedBrand, selectedTag, productos]);
    ...
    // Función para manejar el filtro por tag desde las tarjetas
    const handleTagFilter = (tagId) => {
        setSelectedTag(String(tagId));
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };

    // Componente de tarjeta de tag
    const SectorTagCard = ({ tag }) => {
        const isActive = String(selectedTag) === String(tag.id_tag);
        ...
        const getTagColor = (tag) => {
            ...
            return tag.color || tagParent?.color || '#3B82F6';
        };
        ...
        return (
            ...
            >
                <div 
                    className={`w-36 h-36 flex items-center justify-center rounded-full border-2 overflow-hidden transition-all duration-300 ${
                        isActive
                ...
                >
                    <div className="text-white text-center">
                        <div className="text-2xl font-bold mb-1">
                            {tag.nombre.charAt(0).toUpperCase()}
                        </div>
                        <div className="text-xs font-medium opacity-90">
                            {tag.nombre.length > 8 ? tag.nombre.substring(0, 6) + '...' : tag.nombre}
                        </div>
                    </div>
                </div>
    ...
    // Componente de tarjeta de marca integrado (igual que en Categoria.jsx)
    const SectorBrandCard = ({ brand }) => {
            ...
            <div className={`min-w-screen min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-200'} flex flex-col md:flex-row transition-colors duration-200`}>
                ...
                <div className={`fixed top-0 left-0 h-full z-40 md:relative md:w-1/4 lg:w-1/5 xl:w-1/6 p-4 space-y-4 transform ${isFilterOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out ${isDarkMode ? 'bg-gray-800' : 'bg-white'} overflow-y-auto`}>
                    ...
                    <div className={`p-4 rounded-lg shadow-md ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
                        <h3 className={`font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Filtrar por Tag</h3>
                        <div className="space-y-2">
                            <label className="flex items-center space-x-2 cursor-pointer">
                                ...
                                />
                                <span className={isDarkMode ? 'text-gray-200' : 'text-gray-800'}>Todos los tags</span>
                            </label>
                            
                            {tags && tags.map((tag) => (
                                <label key={tag.id_tag} className="flex items-center space-x-2 cursor-pointer">
                                    <input 
                                        type="radio" 
                                        name="tagFilter" 
                                        value={tag.id_tag}
                                        checked={String(selectedTag) === String(tag.id_tag)}
                                        onChange={handleTagChange}
                                        className="form-radio text-blue-600"
                                    />
                                    <span className={isDarkMode ? 'text-gray-200' : 'text-gray-800'}>{tag.nombre}</span>
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    >
                        <div className="space-y-2">
                            {allTagParents && allTagParents.length > 0 ? (() => {
                                const sectoresOrdenados = [...allTagParents].sort((a, b) => {
                                    const aEsActual = tagParent && tagParent.id_tag_parent === a.id_tag_parent;
                                    const bEsActual = tagParent && tagParent.id_tag_parent === b.id_tag_parent;
                                    if (aEsActual && !bEsActual) return -1;
                                    if (!aEsActual && bEsActual) return 1;
                                    return 0;
                                });
                                
                                return sectoresOrdenados.map((sector, sectorIndex) => {
                                const isCurrentSector = tagParent && tagParent.id_tag_parent === sector.id_tag_parent;
                                return (
                                    <div key={sector.id_tag_parent} className="space-y-1">
                                        {isCurrentSector ? (
                                            <button
                                                ...
                                                style={{ 
                                                    ...
                                                    animationDelay: `${sectorIndex * 0.05}s`
                                                }}
                                            >
                                            ...
                                            <Link
                                                ...
                                                style={{ animationDelay: `${sectorIndex * 0.05}s` }}
                                            >
                                        ...
                                        {isCurrentSector && isTagDropdownOpen && tags && tags.length > 0 && (
                                            <div className="ml-3 space-y-1 animate-slideDown">
                                                {tags.map((tag, tagIndex) => (
                                                    <div
                                                        key={tag.id_tag}
                                                        className={`group block p-2 pl-4 rounded-md transition-all duration-150 transform hover:scale-[1.02] hover:translate-x-1 cursor-pointer ${
                                                            isDarkMode 
                                                        ...
                                                        onClick={() => handleTagFilter(tag.id_tag)}
                                                    >
                                                        <div className="flex items-center">
                                                            <div 
                                                                className={`w-1.5 h-1.5 rounded-full mr-2 transition-all duration-100`}
                                                                style={{ backgroundColor: tag.color || tagParent?.color || '#3B82F6' }}
                                                            ></div>
                ...
                <div className={`flex-1 p-4 transition-colors duration-200 md:ml-0 ${isFilterOpen ? 'ml-0' : 'ml-0'}`}>
                    ...
                    {productos && productos.length > 0 ? (
                        <>
                          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
                            <h1 className={`text-xl md:text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} transition-colors duration-200 mb-2 md:mb-0`}>
                                Sector: {tagParent ? tagParent.nombre : 'Sin sector'}
                        ...
                        <>
                        <h1 className={`text-xl md:text-2xl font-bold mb-4 ${isDarkMode ? 'text-white' : 'text-gray-900'} transition-colors duration-200`}>Sector: {tagParent ? tagParent.nombre : 'Sin sector'}</h1>
                        <div className="text-center py-10">
                          <p className={`text-lg md:text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} transition-colors duration-200`}>No hay productos relacionados a este sector.</p>
                        </div>
                        ...
                        <div className={`p-4 md:p-8 mt-8 transition-colors duration-300 ${
                            ...
                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
                                <h2 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${
                                    ...
                                    Tags disponibles en {tagParent ? tagParent.nombre : 'este sector'}
                                </h2>
                                {selectedTag && (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                                {tags.map((tag) => (
                                    <SectorTagCard key={tag.id_tag} tag={tag} />
                                ))}
                            </div>
                        </div>
                    )}
                        ...
                        <div className={`p-4 md:p-8 mt-8 transition-colors duration-300 ${
                            ...
                            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
                                <h2 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${
                                    ...
                                    Marcas Disponibles en este Sector
                                </h2>
                                {selectedBrand && (
                                    <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                                        isDarkMode ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white'
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                                {marcas.map((marca) => (
                                    <SectorBrandCard key={marca.id_marca} brand={marca} />
                                ))}
                            </div>
                        </div>
                    )}
