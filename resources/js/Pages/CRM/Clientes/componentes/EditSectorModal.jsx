...
export default function EditSectorModal({ isOpen, onClose, sector }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      await axios.post(`/crm/clientes/sectores/${sector.id_sector}`, {
        ...form,
        _method: 'PUT',
      });
      handleClose();
      ...
      alert(err?.response?.data?.message || 'No se pudo actualizar el sector');
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div className={`w-full max-w-xl rounded-xl shadow-xl ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className={`flex items-center justify-between p-6 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="flex items-center">
            ...
            <div>
              <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Editar Sector</h3>
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Modifica los datos del sector seleccionado</p>
            </div>
          </div>
