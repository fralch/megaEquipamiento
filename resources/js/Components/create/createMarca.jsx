...
const Marcas = ({ onSubmit }) => {
  ...
  const [editShowImageBank, setEditShowImageBank] = useState(false);
  ...
  const handleEditChange = (e) => {
    ...
    if (name === 'video_url') {
      setEditVideoPreview(getYouTubeEmbedUrl(value));
    }
  };
  ...
  // Selección de imagen del banco en edición
  const handleEditImageBankSelect = (bankImages) => {
    if (bankImages.length > 0) {
      const selectedImage = bankImages[0];
      setEditPreviewImage(selectedImage.url);
      setEditForm(prev => ({
        ...prev,
        imagen: {
          url: selectedImage.url,
  ...
  const openEditModal = (marca) => {
    setEditForm({
      ...
      imagen: null, // no establecer archivo inicialmente
      video_url: marca.video_url || "",
    });
    setEditPreviewImage(marca.imagen || null);
    setEditVideoPreview(getYouTubeEmbedUrl(marca.video_url || ""));
    setEditModalOpen(true);
  };

  // Enviar actualización de marca
  const handleUpdateMarca = async (e) => {
    e.preventDefault();
    if (!editForm.id_marca) return;
    ...
    if (editForm.imagen) {
      if (editForm.imagen.isFromBank) {
        formData.append('imagen_url', editForm.imagen.url);
        formData.append('imagen_name', editForm.imagen.name);
      } else {
    ...
    try {
      ...
      const response = await axios.post(`/marca/update/${editForm.id_marca}`,
        formData,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        setEditForm({ id_marca: null, nombre: '', descripcion: '', imagen: null, video_url: '' });
        setEditPreviewImage(null);
        setEditVideoPreview(null);
        await fetchMarcas();
      } else {
        throw new Error(response.data?.message || 'Error al actualizar la marca');
      }
    } catch (error) {
      console.error('Error updating marca:', error);
      setMessage({ type: 'error', text: error.response?.data?.message || error.message || 'Error al actualizar la marca.' });
    } finally {
                ...
                <tr key={marca.id_marca} className={`${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} transition-colors duration-200`}>
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm font-medium">
                    ...
                    <button
                      ...
                      className={`ml-3 py-1 px-2 rounded min-h-[32px] transition-colors duration-200 ${
                        isDarkMode 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-2 sm:p-4">
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-4 rounded-t-xl sm:rounded-lg max-w-full sm:max-w-2xl max-h-[90vh] w-full sm:w-auto overflow-auto transition-colors duration-200`}>
            <div className="flex justify-between items-center mb-2">
              <h2 className={`${isDarkMode ? 'text-white' : 'text-gray-900'} text-lg font-bold`}>Editar Marca</h2>
              <button
            ...
            <form onSubmit={handleUpdateMarca}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div className="mb-3 sm:mb-4">
                  <label htmlFor="edit_nombre" className={`block text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} transition-colors duration-200`}>
                    Nombre
                  </label>
                  <input
                    type="text"
                ...
                <div className="mb-3 sm:mb-4">
                  <label htmlFor="edit_descripcion" className={`block text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} transition-colors duration-200`}>
                    Descripción
                  </label>
                  <textarea
                    id="edit_descripcion"
                ...
                <div className="mb-3 sm:mb-4">
                  <label htmlFor="edit_imagen" className={`block text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} transition-colors duration-200`}>
                    Imagen
                  </label>
                  <div className="mt-1 flex flex-col sm:flex-row gap-2">
                    <input
                    ...
                    <button
                      type="button"
                      onClick={() => setEditShowImageBank(true)}
                      className={`px-4 py-3 rounded-md shadow-sm text-sm font-medium transition duration-150 ease-in-out min-h-[44px] flex items-center justify-center whitespace-nowrap ${
                        isDarkMode 
                          ? 'bg-green-600 text-white hover:bg-green-700 border border-green-600' 
                          : 'bg-green-500 text-white hover:bg-green-600 border border-green-500'
                ...
                <div className="mb-3 sm:mb-4">
                  <label htmlFor="edit_video_url" className={`block text-xs sm:text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} transition-colors duration-200`}>
                    URL del Video
                  </label>
                  <input
                    type="url"
              ...
              <div className="mt-4 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setEditModalOpen(false)}
                  className={`px-6 py-3 font-bold rounded-lg min-h-[44px] transition-colors duration-200 ${isDarkMode ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'}`}
                >
                  Cancelar
                </button>
                <button
                  ...
                  className={`px-6 py-3 font-bold rounded-lg min-h-[44px] transition-colors duration-200 ${
                    isDarkMode 
                      ? 'bg-blue-600 hover:bg-blue-700 text-white' 
                      : 'bg-blue-600 hover:bg-blue-700 text-white'
            ...
            {/* Banco de imágenes en edición */}
            {editShowImageBank && (
              <ImageBankModal
                isOpen={editShowImageBank}
                onClose={() => setEditShowImageBank(false)}
                onSelectImages={handleEditImageBankSelect}
              />
            )}
          </div>
        </div>
