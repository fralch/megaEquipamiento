...
// Iconos memoizados como constantes
const ICON_MAP = {
    cart: (
...
const CheckoutTabs = ({ cartItems, onUpdateQuantity, onRemoveItem, userAddresses = [] }) => {
    ...
    // Memoizar función getStepIcon
    const getStepIcon = useCallback((stepId, isCompleted) => {
        if (isCompleted) {
            return COMPLETED_ICON;
        }

        const step = STEPS.find(s => s.id === stepId);
        return ICON_MAP[step?.icon] || <span className="text-sm font-bold">{stepId}</span>;
    }, []);

    const handleStepComplete = useCallback((stepId, data = null) => {
    ...
    // Memoizar contenido del step actual
    ...
    }, [activeStep, cartItems, onUpdateQuantity, onRemoveItem, handleStepComplete, isDarkMode, stepData]);
